...
context("")
test_that("Autenticação", pgm_auth_renv())
...
test_that("Extrair mensagens", pgm_messages(label_ids = pgm_labels(label = "INBOX")$id, num_results = 1))
test_that("Extrair dados das mensagens", 
          pgm_read(thread_id = pgm_threads(label_id = pgm_labels(label = "INBOX")$id, num_results = 1), 
                   message_id = pgm_messages(label_ids = pgm_labels(label = "INBOX")$id, num_results = 1)))
...
test_that("Criar tabela html para o email",
          pgm_tablehtml(obj = data.frame("id"=1, "name"="tablehtml"), width = c(200,300)))
